...
def calculate(LINE_ITERATIONS,track):
    occup = og.OccupancyGrid(track)
    inner_border  = occup.inner_points_raw
    outer_border  = occup.outer_points_raw
    center_line  = occup.intersection_points_raw
    race_line = copy.deepcopy(center_line)  # Use this for centerline being outer bound
    ...
    plt.scatter(*(inner_border).T, c='red', s=0.1, alpha=1)
    plt.scatter(*(outer_border).T, c='blue', s=0.1, alpha=1)
    plt.scatter(*(race_line).T, c='black', s=0.1, alpha=1)
    plt.show()

calculate(1000, "BrandsHatch")